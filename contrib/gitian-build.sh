...
Run this script from the directory containing the absolute, gitian-builder, gitian.signature, and absolute.detached.signatures.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/absolute-community/absolute
...
then
    ...
    git clone https://github.com/absolute-community/gitian.signatures
...
then
	...
	then
	    ...
	    ./bin/gsign -p "$signProg" --signer "$SIGNER" --release ${VERSION}-osx-unsigned --destination ../gitian.sigs/ ../absolute/contrib/gitian-descriptors/gitian-osx.yml
	...
	popd
        ...
        if [[ $commitFiles = true ]]
        then
	    # Commit to gitian.signatures repo
            echo ""
            echo "Committing ${VERSION} Unsigned signatures"
            echo ""
            pushd gitian.signature
            git add ${VERSION}-linux/"${SIGNER}"
            git add ${VERSION}-win-unsigned/"${SIGNER}"
            git add ${VERSION}-osx-unsigned/"${SIGNER}"
            git commit -a -m "Add ${VERSION} unsigned signatures for ${SIGNER}"
            popd
        fi
fi
...
then
	...
	popd
        ...
        then
            ...
            echo ""
            echo "Committing ${VERSION} Signed Signatures"
            echo ""
