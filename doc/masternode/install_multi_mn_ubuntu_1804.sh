...
# check ubuntu version - we need 18.04
if [ -r /etc/os-release ]; then
	. /etc/os-release
	if [ "${VERSION_ID}" != "18.04" ] ; then
		echo "Script needs Ubuntu 18.04, exiting!"
		echo
		exit 1
	fi
else
	echo "Operating system is not Ubuntu 18.04, exiting!"
	echo
	exit 1
fi
...
apt-get install libzmq5 libevent-pthreads-2.1 libminiupnpc10 libevent-2.0-5 -y -qq
