...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "absolute.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("BITCOIND", "absoluted"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            logger.debug("initialize_chain: absoluted started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    ...
    if binary is None:
        binary = os.getenv("BITCOIND", "absoluted")
...
def assert_start_raises_init_error(i, dirname, extra_args=None, expected_msg=None):
    with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
        try:
            ...
            assert 'absoluted exited' in str(e) #node must have shutdown
            ...
            if expected_msg is None:
                assert_msg = "absoluted should have exited with an error"
            else:
                assert_msg = "absoluted should have exited with expected error " + expected_msg
