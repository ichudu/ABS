...
class NULLDUMMYTest(BitcoinTestFramework):
    ...
    def block_submit(self, node, txs, accept = False):
        aip4_activated = self.lastblockheight + 1 >= 432
        block = create_block(self.tip, create_coinbase(self.lastblockheight + 1, dip4_activated=aip4_activated), self.lastblocktime + 1)
        block.nVersion = 4
        for tx in txs:
            tx.rehash()
            block.vtx.append(tx)
